...
# endif
    ...
    int global_x = ceil(input0->shape[0]/4) * 4;
    int global_y = ceil(input0->shape[1]/4) * 4;

    int threadSize = 4;

    size_t global_item_size[2] = {global_x, global_y};
    size_t local_item_size[2] = {threadSize, threadSize};
    // const size_t local_item_size = 4;
    // unsigned int size_a = result->shape[0] * result->shape[1] * result->shape[2]; 
    // const size_t global_item_size[2] = {input0->shape[0], input1->shape[1]};
